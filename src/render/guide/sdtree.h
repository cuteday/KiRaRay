...
public:
    ...
    float eval(vec2f& p, const std::vector<QuadTreeNode>& nodes) const {
    ...
    float pdf(vec2f& p, const std::vector<QuadTreeNode>& nodes) const {
        CHECK(p.x >= 0 && p.x <= 1 && p.y >= 0 && p.y <= 1);
        const int index = childIndex(p);
        if (!(sum(index) > 0)) {
            return 0;
    ...
    int depthAt(vec2f& p, const std::vector<QuadTreeNode>& nodes) const {
        CHECK(p.x >= 0 && p.x <= 1 && p.y >= 0 && p.y <= 1);
        const int index = childIndex(p);
        if (isLeaf(index)) {
            return 1;
    ...
    vec2f sample(Sampler sampler, const std::vector<QuadTreeNode>& nodes) const {
        int index = 0;
    ...
    void record(vec2f& p, float irradiance, std::vector<QuadTreeNode>& nodes) {
        CHECK(p.x >= 0 && p.x <= 1 && p.y >= 0 && p.y <= 1);
    ...
    float computeOverlappingArea(const vec2f& min1, const vec2f& max1, const vec2f& min2, const vec2f& max2) {
        float lengths[2];
        for (int i = 0; i < 2; ++i) {
    ...
    void record(const vec2f& origin, float size, vec2f nodeOrigin, float nodeSize, float value, std::vector<QuadTreeNode>& nodes) {
...
public:
    ...
    void recordIrradiance(vec2f p, float irradiance, float statisticalWeight, EDirectionalFilter directionalFilter) {
    ...
    vec2f sample(Sampler sampler) const {
        if (!(mean() > 0)) {
            return sampler.get2D();
        }
...
public:
    ...
    static vec3f canonicalToDir(vec2f p) {
    ...
    static vec2f dirToCanonical(const vec3f& d) {
    ...
    vec3f sample(Sampler sampler) const {
        return canonicalToDir(sampling.sample(sampler));
    }

    float pdf(const vec3f& dir) const {
...
struct STreeNode {
    ...
    int nodeIndex(vec3f& p) const {
        return children[childIndex(p)];
    }

    DTreeWrapper* dTreeWrapper(vec3f& p, vec3f& size, std::vector<STreeNode>& nodes) {
    ...
    int depth(vec3f& p, const std::vector<STreeNode>& nodes) const {
        CHECK(p[axis] >= 0 && p[axis] <= 1);
        if (isLeaf) {
            return 1;
        }
    ...
    float computeOverlappingVolume(const vec3f& min1, const vec3f& max1, const vec3f& min2, const vec3f& max2) {
        float lengths[3];
        for (int i = 0; i < 3; ++i) {
            lengths[i] = std::max(std::min(max1[i], max2[i]) - std::max(min1[i], min2[i]), 0.0f);
        }
    ...
    void record(const vec3f& min1, const vec3f& max1, vec3f min2, vec3f size2, const DTreeRecord& rec, EDirectionalFilter directionalFilter, EBsdfSamplingFractionLoss bsdfSamplingFractionLoss, std::vector<STreeNode>& nodes) {
...
#pragma omp parallel for
    ...
    void record(const vec3f& p, const vec3f& dTreeVoxelSize, DTreeRecord rec, EDirectionalFilter directionalFilter, EBsdfSamplingFractionLoss bsdfSamplingFractionLoss) {
        float volume = 1;
        for (int i = 0; i < 3; ++i) {
