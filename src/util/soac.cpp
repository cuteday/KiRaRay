...
int main(int argc, char* argv[]) {
	...
	for (const auto& soa : soaTypes) {
		...
		// Iterator [modified]
		printf("    using GetSetIndirector = SOAGetSetIndirector<%s>;\n", workItemName.c_str());
		printf("    using iterator = SOAIterator<%s>;\n", workItemName.c_str());
		// Constructor
		printf("    SOA() = default;\n");
		...
		// GetSetIndirector operator [] non-const ver. definition
		if (!soa.templateType.empty()) printf("template <typename %s>\n", soa.templateType.c_str());
		else printf("template <> \n");
		printf("KRR_CALLABLE void SOAGetSetIndirector<%s>::operator=(const %s& val) {\n",
			   workItemName.c_str(), workItemName.c_str());
		for (const auto &member : soa.members)
			for (int i = 0; i < member.names.size(); ++i) {
				std::string name = member.names[i];
				if (!member.arraySizes[i].empty()) {
					printf("    for (int c = 0; c < %s; ++c)\n", member.arraySizes[i].c_str());
					printf("        m_soa->%s[c][m_index] = val.%s[c];\n", name.c_str(), name.c_str());
				} else printf("    m_soa->%s[m_index] = val.%s;\n", name.c_str(), name.c_str());
			}
		printf("}\n\n");
	}
}