...
int main(int argc, char* argv[]) {
	...
	for (const auto& soa : soaTypes) {
		std::string workItemName;
		if (!soa.templateType.empty())
			workItemName = soa.type + "<" + soa.templateType + ">";
		else workItemName = soa.type;
		if (!soa.templateType.empty())
		...
		printf("    using const_iterator = SOAIterator<value_type>;\n");
		printf("    using reverse_iterator = std::reverse_iterator<iterator>;\n");
		printf("    using const_reverse_iterator = std::reverse_iterator<const_iterator>;\n\n");
		
		printf("	KRR_CALLABLE iterator begin() { return iterator(this, 0); }\n");
		printf("	KRR_CALLABLE const_iterator begin() const { return const_iterator(this, 0); }\n");
		printf("	KRR_CALLABLE iterator end() { return iterator(this, nAlloc); }\n");
		printf("	KRR_CALLABLE const_iterator end() const { return const_iterator(this, nAlloc); }\n");
