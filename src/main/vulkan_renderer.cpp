...
VulkanBaseApp::VulkanBaseApp(const std::string &appName, bool enableValidation)
	: m_appName(appName), m_enableValidation(enableValidation), m_instance(VK_NULL_HANDLE),
	  m_window(nullptr), m_debugMessenger(VK_NULL_HANDLE), m_surface(VK_NULL_HANDLE),
	  m_physicalDevice(VK_NULL_HANDLE), m_device(VK_NULL_HANDLE), m_graphicsQueue(VK_NULL_HANDLE),
	  m_presentQueue(VK_NULL_HANDLE), m_swapChain(VK_NULL_HANDLE), m_vkDeviceUUID(),
	  m_swapChainImages(), m_swapChainFormat(), m_swapChainExtent(), m_swapChainImageViews(),
	  m_shaderFiles(), m_renderPass(), m_pipelineLayout(VK_NULL_HANDLE),
	  m_graphicsPipeline(VK_NULL_HANDLE), m_swapChainFramebuffers(), m_commandPool(VK_NULL_HANDLE),
	  m_commandBuffers(), m_imageAvailableSemaphores(), m_renderFinishedSemaphores(),
	  m_inFlightFences(), m_uniformBuffers(), m_uniformMemory(),
	  m_descriptorSetLayout(VK_NULL_HANDLE), m_descriptorPool(VK_NULL_HANDLE), m_descriptorSets(),
	  m_depthImage(VK_NULL_HANDLE), m_depthImageMemory(VK_NULL_HANDLE),
	  m_depthImageView(VK_NULL_HANDLE), m_currentFrame(0), m_framebufferResized(false) {}
...
static bool findGraphicsQueueIndicies(VkPhysicalDevice device, VkSurfaceKHR surface,
...
static bool hasAllExtensions(VkPhysicalDevice device,
...
static void getSwapChainProperties(VkPhysicalDevice device, VkSurfaceKHR surface,
...
void VulkanBaseApp::createImageViews() {
	...
	for (uint32_t i = 0; i < m_swapChainImages.size(); i++) {
		m_swapChainImageViews[i] = createImageView(m_device, m_swapChainImages[i],
												   m_swapChainFormat, VK_IMAGE_ASPECT_COLOR_BIT);
	}
}
...
static void transitionImageLayout(VulkanBaseApp *app, VkImage image, VkFormat format,
...
void VulkanBaseApp::createDepthResources() {
	...
	m_depthImageView =
		createImageView(m_device, m_depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT);
	transitionImageLayout(this, m_depthImage, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED,
...
#ifdef _VK_TIMELINE_SEMAPHORE
	if (vkCreateSemaphore(m_device, &semaphoreInfo, nullptr, &m_vkPresentationSemaphore) !=
		VK_SUCCESS) {
...
void VulkanBaseApp::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
	...
	allocInfo.allocationSize	   = memRequirements.size;
	allocInfo.memoryTypeIndex =
		findMemoryType(m_physicalDevice, memRequirements.memoryTypeBits, properties);
...
void VulkanBaseApp::createExternalBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
										 VkMemoryPropertyFlags properties,
...
void *VulkanBaseApp::getMemHandle(VkDeviceMemory memory,
	...
	if (fpGetMemoryWin32HandleKHR(m_device, &vkMemoryGetWin32HandleInfoKHR, &handle) !=
		VK_SUCCESS) {
...
void *VulkanBaseApp::getSemaphoreHandle(VkSemaphore semaphore,
	...
	if (fpGetSemaphoreWin32HandleKHR(m_device, &semaphoreGetWin32HandleInfoKHR, &handle) !=
		VK_SUCCESS) {
		throw std::runtime_error("Failed to retrieve handle for buffer!");
	}
...
void VulkanBaseApp::createExternalSemaphore(VkSemaphore &semaphore,
											VkExternalSemaphoreHandleTypeFlagBits handleType) {
	VkSemaphoreCreateInfo semaphoreInfo = {};
	semaphoreInfo.sType					= VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;
...
void VulkanBaseApp::drawFrame() {
	...
	if (vkQueueSubmit(m_graphicsQueue, 1, &submitInfo, m_inFlightFences[currentFrameIdx]) !=
		VK_SUCCESS) {
		throw std::runtime_error("failed to submit draw command buffer!");
	}
