...
void PPGPathTracer::sampleMediumInteraction(int depth) {
	...
	ForAllQueued(
		...
		KRR_DEVICE_LAMBDA(const MediumSampleWorkItem &w) {
			...
			Color T_maj = sampleT_maj(
				ray, w.tMax, sampler, channel,
...
void PPGPathTracer::sampleMediumScattering(int depth) {
		...
		KRR_DEVICE_LAMBDA(const MediumScatterWorkItem &w) {
			...
			if (!thp.isZero() && !thp.hasNaN())
				/* [NOTE] We need to multiply P_path by w.pu. While the PDF of P_light and P_bsdf is
					the same until this vertex, the channel-wise PDF along the path may be
				   different. */
				nextRayQueue(depth)->push(ray, ctx, thp, w.pu, w.pu / ps.pdf, w.depth + 1,
										  w.pixelId, BSDF_SMOOTH);
		},
		gpContext->cudaStream);
}
