...
protected:
	...
	struct PsoCacheKey {
		...
		bool operator==(const PsoCacheKey &other) const {
			return fbinfo == other.fbinfo && shader == other.shader &&
	...
	std::unordered_map<PsoCacheKey, nvrhi::GraphicsPipelineHandle, PsoCacheKey::Hash>
		m_BlitPsoCache;
...
public:
	...
	// Simplified form of BlitTexture that blits the entire source texture, mip 0 slice 0, into the
	// entire target framebuffer using a linear sampler.
	void BlitTexture(nvrhi::ICommandList *commandList, nvrhi::IFramebuffer *targetFramebuffer,
