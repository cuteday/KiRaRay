...
void varySigma() {
  ...
  if (filter_radius > 16) {
    filter_radius = 16;  // clamp to 64 and then negate sign
    g_nFilterSign = -1;
...
class vulkanImageCUDA {
 public:
  void loadImageData(const std::string& filename) {
    ...
    // the window
	auto image_path = File::resolve(filename);

    auto image = Image::createFromFile(image_path);
	image_data = (unsigned int*)image->data();
    ...
    printf("Loaded '%s', %d x %d pixels\n", image_path.string().c_str(), imageWidth,
           imageHeight);
  }
 ...
 private:
  ...
  void initCuda() {
    setCudaVkDevice();
    CUDA_CHECK(cudaStreamCreate(&streamToRun));
  ...
  void cleanup() {
    ...
    for (int i = 0; i < mipLevels; i++) {
      CUDA_CHECK(cudaDestroySurfaceObject(surfaceObjectList[i]));
      CUDA_CHECK(cudaDestroySurfaceObject(surfaceObjectListTemp[i]));
    }

    CUDA_CHECK(cudaFree(d_surfaceObjectList));
    CUDA_CHECK(cudaFree(d_surfaceObjectListTemp));
    CUDA_CHECK(cudaFreeMipmappedArray(cudaMipmappedImageArrayTemp));
    CUDA_CHECK(cudaFreeMipmappedArray(cudaMipmappedImageArrayOrig));
    CUDA_CHECK(cudaFreeMipmappedArray(cudaMipmappedImageArray));
    CUDA_CHECK(cudaDestroyTextureObject(textureObjMipMapInput));
    CUDA_CHECK(cudaDestroyExternalMemory(cudaExtMemImageBuffer));
    CUDA_CHECK(cudaDestroyExternalSemaphore(cudaExtCudaUpdateVkSemaphore));
    CUDA_CHECK(cudaDestroyExternalSemaphore(cudaExtVkUpdateCudaSemaphore));
  ...
  void createGraphicsPipeline() {
    auto vertShaderCode = readFile("src/misc/samples/simple-vulkan/shaders/vert.spv");
    auto fragShaderCode = readFile("src/misc/samples/simple-vulkan/shaders/frag.spv");
  ...
  void createImage(uint32_t width, uint32_t height, VkFormat format,
    ...
    printf("Total memory size required: %lld\n", totalImageMemSize);
  ...
  void cudaVkImportSemaphore() {
    ...
    CUDA_CHECK(cudaImportExternalSemaphore(&cudaExtCudaUpdateVkSemaphore,
...
#endif
    externalSemaphoreHandleDesc.flags = 0;
    CUDA_CHECK(cudaImportExternalSemaphore(&cudaExtVkUpdateCudaSemaphore,
                                                &externalSemaphoreHandleDesc));
  ...
  void cudaVkImportImageMem() {
    ...
    CUDA_CHECK(cudaImportExternalMemory(&cudaExtMemImageBuffer,
    ...
    CUDA_CHECK(cudaMallocMipmappedArray(&cudaMipmappedImageArrayTemp,
                                             &formatDesc, extent, mipLevels));
    CUDA_CHECK(cudaMallocMipmappedArray(&cudaMipmappedImageArrayOrig,
                                             &formatDesc, extent, mipLevels));
    ...
    CUDA_CHECK(cudaCreateTextureObject(&textureObjMipMapInput, &resDescr,
                                            &texDescr, NULL));

    CUDA_CHECK(cudaMalloc((void**)&d_surfaceObjectList,
                               sizeof(cudaSurfaceObject_t) * mipLevels));
    CUDA_CHECK(cudaMalloc((void**)&d_surfaceObjectListTemp,
                               sizeof(cudaSurfaceObject_t) * mipLevels));

    CUDA_CHECK(cudaMemcpy(d_surfaceObjectList, surfaceObjectList.data(),
                               sizeof(cudaSurfaceObject_t) * mipLevels,
  ...
  void cudaUpdateVkImage() {
    ...
    int nthreads  = 128;
	int nblocks_x = (imageHeight + nthreads - 1) / nthreads;
	int nblocks_y = (imageWidth + nthreads - 1) / nthreads;

    /*Perform 2D box filter on image using CUDA */
    d_boxfilter_rgba_x<<<nblocks_x, nthreads, 0, streamToRun>>>(
        d_surfaceObjectListTemp, textureObjMipMapInput, imageWidth, imageHeight,
        mipLevels, filter_radius);

    d_boxfilter_rgba_y<<<nblocks_y, nthreads, 0, streamToRun>>>(
  ...
  void updateUniformBuffer() {
	...
	ubo.model.topLeftCorner<3, 3>() =
		Eigen::AngleAxis<float>(radians(135.0f), Vector3f::UnitZ()).matrix();
	...
	ubo.proj = perspective(radians(45.0f), swapChainExtent.width / (float) swapChainExtent.height,
						   0.1f, 10.0f);
    ubo.proj(1, 1) *= -1;
  ...
  static std::vector<char> readFile(const std::string& filename) {
    auto file_path = File::resolve(filename.c_str());
...
extern "C" int main(int argc, char *argv[]) {
  std::string image_filename = "src/misc/samples/simple-rhi/assets/emoticon_004.png";
