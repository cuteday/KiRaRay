...
public:
	VulkanCudaSineWave(size_t width, size_t height)
		: VulkanBaseApp("vulkanCudaSineWave", ENABLE_VALIDATION), m_heightBuffer(VK_NULL_HANDLE),
		  m_xyBuffer(VK_NULL_HANDLE), m_indexBuffer(VK_NULL_HANDLE), m_heightMemory(VK_NULL_HANDLE),
		  m_xyMemory(VK_NULL_HANDLE), m_indexMemory(VK_NULL_HANDLE), m_ubo(), m_sim(width, height),
		  m_stream(0), m_vkWaitSemaphore(VK_NULL_HANDLE), m_vkSignalSemaphore(VK_NULL_HANDLE),
		  m_cudaWaitSemaphore(), m_cudaSignalSemaphore(), m_cudaTimelineSemaphore(),
		  m_cudaVertMem(), m_cudaHeightMap(nullptr), m_lastFrame(0) {
	...
	void fillRenderingCommandBuffer(VkCommandBuffer &commandBuffer) {
		...
		vkCmdBindVertexBuffers(commandBuffer, 0, sizeof(vertexBuffers) / sizeof(vertexBuffers[0]),
							   vertexBuffers, offsets);
		vkCmdBindIndexBuffer(commandBuffer, m_indexBuffer, 0, VK_INDEX_TYPE_UINT32);
		vkCmdDrawIndexed(commandBuffer,
						 (uint32_t) ((m_sim.getWidth() - 1) * (m_sim.getHeight() - 1) * 6), 1, 0, 0,
						 0);
	}
	...
	void initVulkanApp() {
		...
		importCudaExternalMemory((void **) &m_cudaHeightMap, m_cudaVertMem, m_heightMemory,
								 nVerts * sizeof(*m_cudaHeightMap), getDefaultMemHandleType());
	...
	void importCudaExternalSemaphore(cudaExternalSemaphore_t &cudaSem, VkSemaphore &vkSem,
	...
	void drawFrame() {
		...
		float frame_time =
			std::chrono::duration<float, std::chrono::seconds::period>(currentTime - m_lastTime)
				.count();
