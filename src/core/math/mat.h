...
namespace krr {
  namespace math {
    ...
    template<typename T> struct mat2_t
    {
      ...
      inline __both__ mat2_t           ( const mat2_t& other ) { vx = other.vx; vy = other.vy; }
      inline __both__ mat2_t& operator=( const mat2_t& other ) { vx = other.vx; vy = other.vy; return *this; }

      template<typename L1> inline __both__ mat2_t( const mat2_t<L1>& s ) : vx(s.vx), vy(s.vy) {}
    ...
    template<typename T> __both__ inline mat2_t<T> operator -( const mat2_t<T>& a ) { return mat2_t<T>(-a.vx,-a.vy); }
    template<typename T> __both__ inline mat2_t<T> operator +( const mat2_t<T>& a ) { return mat2_t<T>(+a.vx,+a.vy); }
    template<typename T> __both__ inline mat2_t<T> rcp       ( const mat2_t<T>& a ) { return a.inverse(); }

    
    // Binary Operators
    

    template<typename T> inline mat2_t<T> operator +( const mat2_t<T>& a, const mat2_t<T>& b ) { return mat2_t<T>(a.vx+b.vx,a.vy+b.vy); }
    template<typename T> inline mat2_t<T> operator -( const mat2_t<T>& a, const mat2_t<T>& b ) { return mat2_t<T>(a.vx-b.vx,a.vy-b.vy); }

    template<typename T> inline mat2_t<T> operator*(const typename T::scalar_t & a, const mat2_t<T>& b) { return mat2_t<T>(a*b.vx, a*b.vy); }
    template<typename T> inline T               operator*(const mat2_t<T>& a, const T              & b) { return b.x*a.vx + b.y*a.vy; }
    template<typename T> inline mat2_t<T> operator*(const mat2_t<T>& a, const mat2_t<T>& b) { return mat2_t<T>(a*b.vx, a*b.vy); }

    template<typename T> inline mat2_t<T> operator/(const mat2_t<T>& a, const typename T::scalar_t & b) { return mat2_t<T>(a.vx/b, a.vy/b); }
    template<typename T> inline mat2_t<T> operator/(const mat2_t<T>& a, const mat2_t<T>& b) { return a * rcp(b); }

    template<typename T> inline mat2_t<T>& operator *=( mat2_t<T>& a, const mat2_t<T>& b ) { return a = a * b; }
    template<typename T> inline mat2_t<T>& operator /=( mat2_t<T>& a, const mat2_t<T>& b ) { return a = a / b; }
    ...
    template<typename T> inline bool operator ==( const mat2_t<T>& a, const mat2_t<T>& b ) { return a.vx == b.vx && a.vy == b.vy; }
    template<typename T> inline bool operator !=( const mat2_t<T>& a, const mat2_t<T>& b ) { return a.vx != b.vx || a.vy != b.vy; }
    ...
    template<typename T> static std::ostream& operator<<(std::ostream& cout, const mat2_t<T>& m) {
    ...
    struct  mat3_t
    {
      ...
      inline __both__ mat3_t& operator=( const mat3_t& other ) { vx = other.vx; vy = other.vy; vz = other.vz; return *this; }

      template<typename L1> inline __both__ mat3_t( const mat3_t<L1>& s ) : vx(s.vx), vy(s.vy), vz(s.vz) {}
    ...
    template<typename T> inline __both__ mat3_t<T> operator -( const mat3_t<T>& a ) { return mat3_t<T>(-a.vx,-a.vy,-a.vz); }
    template<typename T> inline __both__ mat3_t<T> operator +( const mat3_t<T>& a ) { return mat3_t<T>(+a.vx,+a.vy,+a.vz); }
    template<typename T> inline __both__ mat3_t<T> rcp       ( const mat3_t<T>& a ) { return a.inverse(); }
...
// #endif
    ...
    template<typename T> inline __both__ mat3_t<T> frame(const T& N, const T& dxi)
    {
    ...
    template<typename T> inline __both__ mat3_t<T> clamp(const mat3_t<T>& space) {
    ...
    template<typename T> inline __both__ mat3_t<T> operator +( const mat3_t<T>& a, const mat3_t<T>& b ) { return mat3_t<T>(a.vx+b.vx,a.vy+b.vy,a.vz+b.vz); }
    template<typename T> inline __both__ mat3_t<T> operator -( const mat3_t<T>& a, const mat3_t<T>& b ) { return mat3_t<T>(a.vx-b.vx,a.vy-b.vy,a.vz-b.vz); }

    template<typename T> inline __both__ mat3_t<T> operator*(const typename T::scalar_t & a, const mat3_t<T>& b) { return mat3_t<T>(a*b.vx, a*b.vy, a*b.vz); }
    template<typename T> inline __both__ T               operator*(const mat3_t<T>& a, const T              & b) { return b.x*a.vx + b.y*a.vy + b.z*a.vz; }
    template<typename T> inline __both__ mat3_t<T> operator*(const mat3_t<T>& a, const mat3_t<T>& b) { return mat3_t<T>(a*b.vx, a*b.vy, a*b.vz); }

    template<typename T> __both__ inline mat3_t<T> operator/(const mat3_t<T>& a, const typename T::scalar_t & b) { return mat3_t<T>(a.vx/b, a.vy/b, a.vz/b); }
  
    template<typename T> __both__ inline mat3_t<T> operator/(const mat3_t<T>& a, const mat3_t<T>& b) { return a * rcp(b); }

    template<typename T> inline mat3_t<T>& operator *=( mat3_t<T>& a, const mat3_t<T>& b ) { return a = a * b; }
    template<typename T> inline mat3_t<T>& operator /=( mat3_t<T>& a, const mat3_t<T>& b ) { return a = a / b; }

    template<typename T> inline __both__ T xfmPoint (const mat3_t<T>& s, const T& a) { return madd(T(a.x),s.vx,madd(T(a.y),s.vy,T(a.z*s.vz))); }
    template<typename T> inline __both__ T xfmVector(const mat3_t<T>& s, const T& a) { return madd(T(a.x),s.vx,madd(T(a.y),s.vy,T(a.z*s.vz))); }
    template<typename T> inline __both__ T xfmNormal(const mat3_t<T>& s, const T& a) { return xfmVector(s.inverse().transposed(),a); }

    
    /// Comparison Operators
    

    template<typename T> inline bool operator ==( const mat3_t<T>& a, const mat3_t<T>& b ) { return a.vx == b.vx && a.vy == b.vy && a.vz == b.vz; }
    template<typename T> inline bool operator !=( const mat3_t<T>& a, const mat3_t<T>& b ) { return a.vx != b.vx || a.vy != b.vy || a.vz != b.vz; }
    ...
    template<typename T> inline std::ostream& operator<<(std::ostream& cout, const mat3_t<T>& m) {
