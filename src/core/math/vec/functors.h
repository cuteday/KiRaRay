...
#undef _define_op_assign_operator
	...
	// math vector - float operator overload
#define _define_vector_number_operator(fname, fct, NT)                         \
	template<typename T>												\
	inline __both__ vec_t<T,1> fname(const vec_t<T,1> &a, const NT &b)    \
	{                                                                   \
	  return vec_t<T,1>(fct(a.x,b));                                    \
	}                                                                   \
																		\
	template<typename T>												\
	inline __both__ vec_t<T,2> fname(const vec_t<T,2> &a, const NT &b)    \
	{                                                                   \
	  return vec_t<T,2>(fct(a.x,b),                                     \
						fct(a.y,b));                                    \
	}                                                                   \
																		\
	template<typename T>												\
	inline __both__ vec_t<T,3> fname(const vec_t<T,3> &a, const NT &b)    \
	{                                                                   \
	  return vec_t<T,3>(fct(a.x,b),                                     \
						fct(a.y,b),                                     \
						fct(a.z,b));                                    \
	}       
												
	_define_vector_number_operator(vpow, pow, float);
